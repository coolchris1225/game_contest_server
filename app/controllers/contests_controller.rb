class ContestsController < ApplicationController
  ...
  def destroy
    ...
    if current_user?(@contest.user) #|| current_user.admin?
      ...
      flash[:danger] = "Cannot delete contest."
  ...
  def update
    if @contest.update_attributes(acceptable_params)
      flash[:success] = "Referee #{@contest.name} has been updated!"
      redirect_to @contest
    else
      render 'edit'
    end
  ...
  private
   ...
   def ensure_correct_user
    ...
    def ensure_user_logged_in
     redirect_to login_path, flash: { :warning => "Error: please log in!" } unless logged_in? 
    end
    
    def ensure_contest_creator 
      redirect_to root_path, flash: { :danger => "Error: You are not a contest creator!" } unless current_user.contest_creator?
    end
